...
p.fits=ggplot(data=all_fires.sdc,aes(x=width,y=prop.hs))+
  ...
  annotate("text", x = c(0,200,350,950), y = c(0.1,0.1,0.1,0.1), label = unique(all_fires.sdc$q.name),size=3)+
...
####S1e. Create decay profile for each set of circles####
#Initialize
buf_inc= 10 #Buffer increment in m
buf_max= 1000 #Max buffer width is 1000m; needs to be a multiple of buf_inc
#Run the decay function on each "fire"
...
p.fits=ggplot(data=all_fires.sdc.supp,aes(x=width,y=prop.hs))+
  ...
  scale_color_manual(values=plot.colors)+
  geom_path(aes(x=width,y=1/(10^(q*width)),group=name,col=name))+
  annotate("text", x = c(950,700,400), y = c(0.35,0.1,0.1), label = unique(all_fires.sdc.supp$q.name),size=3)+
