...
for(r in 1:nrow(c.df)){
  ...
  c.plots[[r]] <-
    ...
    annotate("text",x=0,y=5,label=c.rad[[r]],hjust=0,vjust=0)+
    lims(x=c(-1000,1000),y=c(-1000,1000))+
    labs(title=paste0("sdc = ",c.df[r,"sdc"], 
                      "\n [log(sdc) = ", round(log(as.double(c.df[r,"sdc"])),3),"]"))+
    theme_bw()+
    theme(plot.title = element_text(hjust = 0.5))
}

png(file = paste0("./Manuscripts/MS2- Trends/Figures/FigS1_wxyz",Sys.Date(),".png"),width=8,height=12,units="in",res=200)
